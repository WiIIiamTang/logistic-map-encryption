...
class LogisticMapPixel(ImageEncryptDecrypter):
    def __init__(self, name: str='LogisticMapPixelEncrypter', r_min=3.87, r_max=3.999999, offset_range: tuple=(0, 255)):
        super().__init__(name)
    ...
    def encrypt_action(self, image: Image, key=None) -> tuple:
        ...
        for i in tqdm(range(size[0])):
            for j in range(size[1]):
                tmp = []
                for k in pixels[i, j]:
                    keypart = self._forward_l(k, self.sysrand.randrange(10, 15), (self.sysrand.random() * (self.r_max-self.r_min)) + self.r_min)
